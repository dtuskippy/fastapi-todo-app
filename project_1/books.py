...
# async not needed in fastapi, but used for teaching purposes
@app.get("/")
async def first_api():
    return {"message": "Gizmo rules!"}
    # return "Gizmo rules!"
    # return BOOKS
...
@app.get("/books/book_author/{book_author}/")
...
@app.get("/books/{book_author}/")
async def fetch_author_by_query(book_author: str):
    books_to_return = []
    for book in BOOKS:
        if book.get('author').casefold() == book_author.casefold():
         books_to_return.append(book)
